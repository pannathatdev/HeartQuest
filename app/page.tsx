...
export default function Home() {
  ...
  const downloadStory = async () => {
    if(!published) return;
    if(!published)return;const canvas=document.createElement("canvas");canvas.width=1080;canvas.height=1920;const ctx=canvas.getContext("2d")!,bg=new Image();bg.src="/ig-story-bg-v1.png";await bg.decode();ctx.drawImage(bg,0,0,1080,1920);ctx.fillStyle="#150f25a8";ctx.fillRect(0,0,1080,1920);ctx.textAlign="center";ctx.fillStyle="#ff9fba";ctx.font="700 28px 'Press Start 2P',monospace";ctx.fillText("HEARTQUEST",540,150);ctx.fillStyle="#fff";ctx.font="700 64px 'Chakra Petch',sans-serif";ctx.fillText("ภารกิจหัวใจถูกสร้างแล้ว",540,305);ctx.font="500 42px 'Chakra Petch',sans-serif";ctx.fillStyle="#f5dce8";ctx.fillText(`${creator||"ใครบางคน"}  →  ${partner||"คนพิเศษ"}`,540,380);ctx.fillStyle="#21182de8";ctx.beginPath();ctx.roundRect(105,500,870,655,42);ctx.fill();ctx.strokeStyle="#ffffff55";ctx.lineWidth=3;ctx.stroke();ctx.fillStyle="#ff769d";ctx.font="110px sans-serif";ctx.fillText("♥",540,675);ctx.fillStyle="#fff";ctx.font="700 46px 'Chakra Petch',sans-serif";ctx.fillText(`${questPlan.length} ด่าน · ${memories.filter(Boolean).length} ความทรงจำ`,540,770);ctx.font="400 31px 'Chakra Petch',sans-serif";ctx.fillStyle="#e8dce7";ctx.fillText("มีข้อความลับรออยู่ที่ปลายทาง",540,830);const qrUrl=await QRCode.toDataURL(published.gameUrl,{width:260,margin:1,color:{dark:"#24182fff",light:"#fff5f8ff"}}),qr=new Image();qr.src=qrUrl;await qr.decode();ctx.fillStyle="#fff5f8";ctx.beginPath();ctx.roundRect(400,875,280,280,24);ctx.fill();ctx.drawImage(qr,410,885,260,260);ctx.fillStyle="#fff";ctx.font="700 36px 'Chakra Petch',sans-serif";ctx.fillText("สแกนเพื่อเริ่มภารกิจ",540,1240);ctx.fillStyle="#f2dbe4";ctx.font="400 27px 'Chakra Petch',sans-serif";ctx.fillText("หรือเพิ่ม Link Sticker แล้ววางลิงก์เกม",540,1295);ctx.fillStyle="#ffffffd9";ctx.font="600 25px 'Chakra Petch',sans-serif";ctx.fillText("สร้างเพื่อคุณเพียงคนเดียว ✦",540,1740);const a=document.createElement("a");a.download=`heartquest-${partner||"story"}.png`;a.href=canvas.toDataURL("image/png");a.click();
  };
