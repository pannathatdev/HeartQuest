...
export async function POST(request:Request){
  let user;try{user=await requireUser(request)}catch(e){if(e instanceof Response)return e;throw e}
  ...
  const game=await env.DB.prepare("SELECT id FROM games WHERE slug = ? AND owner_user_id = ?").bind(String(body.slug||""),user.id).first<{id:string}>();if(!game)return Response.json({error:"ไม่พบเกมของคุณ"},{status:404});
  ...
  await env.DB.prepare("INSERT INTO payments (id, game_id, package_id, amount_satang, provider, status, customer_contact, created_at, user_id) VALUES (?, ?, ?, ?, 'stripe', 'creating_checkout', ?, ?, ?)").bind(paymentId,game.id,body.packageId,pack.amountSatang,String(body.contact||"").slice(0,120)||user.email,Date.now(),user.id).run();
