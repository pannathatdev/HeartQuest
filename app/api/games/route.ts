...
export async function POST(request: Request) {
  try {
    ...
    const allowed=new Set(["npc","key","collect","quiz","match","timeline","catch","rhythm","jigsaw","scratch","secret","ending"]);const questPlan=Array.isArray(body.questPlan)?body.questPlan.slice(0,8).map((s:any)=>({id:clean(s?.id,50)||crypto.randomUUID(),type:allowed.has(clean(s?.type,20))?clean(s.type,20):"collect",title:clean(s?.title,80),instruction:clean(s?.instruction,180),imageUrl:clean(s?.imageUrl,140000),target:Math.max(4,Math.min(20,Number(s?.target)||8)),speed:["easy","normal","hard"].includes(clean(s?.speed,10))?clean(s.speed,10):"normal",question:clean(s?.question,140),answer:clean(s?.answer,80),decoys:Array.isArray(s?.decoys)?s.decoys.slice(0,3).map((x:unknown)=>clean(x,80)):[]})):[];
