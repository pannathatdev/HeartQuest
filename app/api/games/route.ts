...
export async function POST(request: Request) {
  try {
    ...
    const now = Date.now();
    await env.DB.prepare("INSERT INTO games (id, slug, edit_token, creator_name, partner_name, message, theme, youtube_url, memories_json, referral_code, referred_by, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)")
      .bind(id, slug, editToken, creatorName, partnerName, message, clean(body.theme, 20) || "sunset", clean(body.youtubeUrl, 300) || null, JSON.stringify(memories), referralCode, referredBy, now).run();
